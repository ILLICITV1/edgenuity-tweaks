(function() {
  ...
  // --- New code for Search Selection button ---
  window.openedWindows = []; // Array to store opened windows

  // Function to create a button with specific properties
  function buildMenuButton(text, id, onclick) {
    const button = document.createElement('button');
    button.innerText = text;
    button.id = id;
    button.style.cssText = 'position: fixed; top: 60px; left: 50%; transform: translateX(-50%); z-index: 10000; border: none; background-color: #007bff; color: white; padding: 10px 20px; border-radius: 30px; font-size: 16px; cursor: pointer; box-shadow: 0px 4px 6px rgba(0,0,0,0.1); transition: background-color 0.3s,box-shadow 0.3s;';
    button.onmouseover = () => {
  ...
  function search() {
    ...
    // Check for selection in iframes
  ...
  // Create the Search Selection button
  buildMenuButton("Search Selection", "googlebutton", search);
